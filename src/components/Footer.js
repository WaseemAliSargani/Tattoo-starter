...
const Footer = () => {
  ...
  return <footer className='section bg-dark'>
    <div className="container mx-auto">
      ...
      className='text-white flex flex-col lg:flex-row lg:justify-center gap-x-5 gap-y-14 '>
        {/* about */}
        <motion.div 
        variants={fadeIn("up")}  
        ...
        variants={fadeIn("up")}
        className='flex-1'>
          <div className='font-primary text-xl tracking-[0.08em] mb-6'> {program.title} </div>
          <ul className='flex flex-col gap-y-4 text-[#dbdbdb]'>
        ...
        className='flex-1'>
          ...
          <form className='flex justify-between items-start border-b border-[#b6b6b6'>
            <input className='outline-none placeholder:text-base italic placeholder:capitalize bg-transparent w-full' placeholder={newsletter.form.placeholder} type="email"  />
            <button className='text-2xl cursor-pointer ml-4 text-white' type='submit'> {newsletter.form.icon} </button>
          </form>
        </motion.div>
      </motion.div>
    </div>
